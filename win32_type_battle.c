...
Win32ReadFile(char *Path, transient_allocator *Memory)
{
    Assert(Path);
    ...
    Result.Content = AllocateTransient(Memory, FileInfo.st_size);
    if (Result.Content)
    {
        DWORD BytesRead = 0;
...
internal void
Win32ListDirectoryFiles(char* Directory, char* Extension,
...
internal void
Win32GetClientSize(HWND Window, i32 *Width, i32* Height)
{
    RECT Rect;
...
// C Files
#include "math/sim_math.c"
#include "memory/transient.c"
#include "rendering/opengl/renderer.c"
